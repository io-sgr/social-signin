...
public final class GoogleSignInService implements Closeable {
     ...
     *         This parameter defaults to online. If your application needs to refresh access tokens when the user is not present at the browser, then use
     *         offline.
     ...
     *         Passing this hint will either pre-fill the email box on the sign-in form or select the proper multi-login session, thereby simplifying the
     *         login flow.
     * @param includeGrantedScopes
     *         If this is provided with the value true, and the authorization request is granted, the authorization will include any previous authorizations
     *         granted to this user/application combination for other scopes.
     * @param extraScopes
     *         Space-delimited list of extra scopes beside of "profile email" for the OAuth request
     * @param prompts
     *         Space-delimited, case-sensitive list of prompts to present the user. If you don't specify this parameter, the user will be prompted only the
        ...
        try {
            while (!Thread.interrupted()) {
                ...
                try {
                    ...
                    LOGGER.warn(
                            String.format("Unable to get access token because of [%s]%s, retry after %d second(s).", err.getName(), err.getErrorDescription(),
                                    interval));
                }
        ...
        try {
            while (!Thread.interrupted()) {
                ...
                try {
                    ...
                    OAuthError err = e.getError();
                    LOGGER.warn(String.format("Unable to refresh access token because of [%s]%s, retry after %d second(s).", err.getName(),
                            err.getErrorDescription(), interval));
                }
                TimeUnit.SECONDS.sleep(interval++);
                retryCnt++;
            }
        ...
        try {
            while (!Thread.interrupted()) {
                ...
                try {
                    ...
                    OAuthError err = e.getError();
                    LOGGER.warn(String.format("Unable to revoke token because of [%s]%s, retry after %d second(s).", err.getName(), err.getErrorDescription(),
                            interval));
                }
                TimeUnit.SECONDS.sleep(interval++);
                retryCnt++;
    ...
    /**
     * @return The provider name of this OAuth server
     */
